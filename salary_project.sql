...
select EmployeeName,JobTitle from salaries_data;


-- 3	Show the number of employees in the table.
select count(*) from salaries_data;

-- 4	Show the unique job titles in the table.
select distinct JobTitle from salaries_data;
-- 5	Show the job title and overtime pay for all employees with 
-- overtime pay greater than 50000.
select JobTitle,OvertimePay from salaries_data
where OvertimePay > 50000;

-- 6	Show the average base pay for all employees.
select AVG(BasePay) as "Avg BasePay" from salaries_data;

-- 7	Show the top 10 highest paid employees.
select EmployeeName,TotalPay from salaries_data
order by TotalPay desc
limit 10;
-- 8	Show the average of BasePay, OvertimePay, and OtherPay for each employee:
select EmployeeName, (BasePay + OvertimePay + OtherPay)/3 as avg_of_bp_op_otherpay from salaries_data;
...
select JobTitle, AVG(BasePay) as "avgbasepay" from salaries_data
group by JobTitle
...
select count(*) from salaries_data
where OvertimePay =0;
delete from salaries_data
where OvertimePay =0;
select count(*) from salaries_data
where OvertimePay =0;

-- 19.To Print the first 3 character of Employee name
select EmployeeName,Substring(EmployeeName,1,5) from Salaries_data;

-- 20.To print a Employee name ascending order
Select * from salaries_data order by EmployeeName asc;

-- 21.Show the columns present in the salary table
Show columns from salaries_data;
